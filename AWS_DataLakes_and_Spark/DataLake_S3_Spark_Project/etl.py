...
def process_log_data(spark, input_data, output_data):
    ...
    #print(df.columns)
    filtered_df = df.filter(df.page == 'NextSong').select('ts', 'userId', 'level', 'song', 'artist','sessionId', 'location', 'userAgent')
    #filtered_df.createOrReplaceTempView("logdata")
    ...
    filtered_df = filtered_df.withColumn('timestamp', get_timestamp(filtered_df.ts))
    
    # create datetime column from original timestamp column
    get_datetime = udf(lambda x: str(datetime.fromtimestamp(int(x) / 1000)))
    filtered_df = filtered_df.withColumn('datetime', get_datetime(filtered_df.ts))
    
    # extract columns to create time table
    time_table = filtered_df.select('datetime') \
                        .withColumn('start_time', filtered_df.datetime) \
    ...
    filtered_df = filtered_df.alias('log_df')
